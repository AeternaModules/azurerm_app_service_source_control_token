data "azurerm_key_vault_secret" "token" {
  for_each     = { for k, v in var.app_service_source_control_tokens : k => v if v.token_key_vault_id != null && v.token_key_vault_secret_name != null }
...
data "azurerm_key_vault_secret" "token_secret" {
  for_each     = { for k, v in var.app_service_source_control_tokens : k => v if v.token_secret_key_vault_id != null && v.token_secret_key_vault_secret_name != null }
